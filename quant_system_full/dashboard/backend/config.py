...
class Settings:
    ...
    # Data source fixed: yahoo_api for selection, tiger for analysis/trading
    # No auto-switching - enforce single source per phase
    data_source: str = os.getenv("DATA_SOURCE","yahoo_api")
    
    # Yahoo Finance API settings
    yahoo_api_timeout: float = float(os.getenv("YAHOO_API_TIMEOUT", "10.0"))
    yahoo_api_retries: int = int(os.getenv("YAHOO_API_RETRIES", "3"))
    yahoo_api_retry_delay: float = float(os.getenv("YAHOO_API_RETRY_DELAY", "1.0"))
    
    # Multi-asset data manager settings
    ...
    selection_task_interval: int = int(os.getenv("SELECTION_TASK_INTERVAL", "1800"))  # 30 minutes
