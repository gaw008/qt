...
class AutoTradingEngine:
    ...
    def get_buying_power(self):
        ...
        try:
            ...
            # 获取现金余额而不是购买力，以避免过度杠杆化
            cash_balance = getattr(summary, 'cash', None)
            if cash_balance is not None and cash_balance > 0:
                # 使用真实现金余额，确保安全交易
                actual_cash = float(cash_balance)
                append_log(f"[AUTO_TRADING] Real cash balance: ${actual_cash:,.2f}")
                return actual_cash
            else:
                # 如果无法获取现金余额，使用保守的购买力限制
                buying_power = getattr(summary, 'buying_power', self.max_position_value)
                # 使用保守的购买力限制，避免过度杠杆
                safe_buying_power = min(buying_power * 0.2, self.max_position_value)  # 只使用20%购买力
                append_log(f"[AUTO_TRADING] Conservative buying power: ${safe_buying_power:,.2f}")
                return float(safe_buying_power)
    ...
    def analyze_trading_opportunities(self, current_positions: List[Dict],
        ...
        for symbol, rec_pos in recommended_buys.items():
            if symbol not in current_symbols:
                ...
                # 使用实际购买力计算买入数量，限制最大为20%现金余额，分散风险
                max_allocation = min(available_buying_power * 0.20, self.max_position_value)
