...
### 安装Python
为获得最好的支持，减少兼容性问题，建议安装最新版本的Python。早于Python 3.4的版本可能不能正常运行。您可以在控制台中输入`python3 -V`来进行版本检查或验证安装
...
|  信息   | 是否必填  | 说明 |
|  ----  | ----  | ----| 
| RSA公钥  | 是 | 使用RSA公钥，是为了确保用户的接口请求安全，通过RSA双向签名验证机制，防止接口请求被恶意篡改，关于RSA的生成，请参考快速入门-准备工作中[生成RSA密钥](/zh/python/quickStart/prepare.html#生成rsa密钥)部分|
...
- Windows下的安装较为繁琐，请参考[OpenSSL官方网站](https://www.openssl.org)，或酌情使用在线生成工具。具体工具请搜索关键词 “RSA证书在线生成”，或者直接通过第三方下载.exe安装工具：[http://slproweb.com/products/Win32OpenSSL.html](http://slproweb.com/products/Win32OpenSSL.html)
...
1. 首先生成RSA私钥，使用以下命令生成私钥文件会保存在当前目录。如果选择使用Python SDK，使用此命令生成的rsa_private_key.pem作为私钥即可
...
注意： 
1. 生成的密钥需为1024位
2. Java 与 Python SDK 使用的私钥格式不同，Python 使用的私钥为 PKCS#1 格式。若使用SDK遇到问题时，注意先检查私钥的格式是否正确

生成RSA公私钥后，需将RSA公钥(默认文件名为rsa_public_key.pem)提交给开放平台，将RSA私钥（默认文件名为rsa_private_key.pem）妥善保存，后续程序调用API时将会用到
